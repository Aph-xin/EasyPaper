...
# Base URL for internal agent-to-agent HTTP calls (Typesetter, Reviewer, Planner, VLM).
# When running via start.sh on port 8004, set EASYPAPER_BASE_URL=http://127.0.0.1:8004 so
# calls hit the same process instead of port 8000 (which may return 405 or be another service).
EASYPAPER_BASE_URL = os.environ.get("EASYPAPER_BASE_URL", "http://127.0.0.1:8000").rstrip("/")
...
class MetaDataAgent(ReActAgent):
    ...
    def __init__(self, config: ModelConfig, tools_config: Optional[ToolsConfig] = None):
        ...
        # Base directory for all paper outputs.
        # Prefer environment override so deployments can control where PDFs land.
        results_dir_env = os.getenv("EASYPAPER_RESULTS_DIR")
        if results_dir_env:
            self.results_dir = Path(results_dir_env).expanduser().resolve()
        else:
...
"""
        ...
        if "abstract" in sections:
            abstract_content = self._strip_document_preamble_from_section(sections["abstract"])
            latex += r"\begin{abstract}" + "\n"
            latex += abstract_content + "\n"
        ...
        for section_type in section_order:
            if section_type in sections and sections[section_type]:
                ...
                # Strip document preamble/structure from section (Writer may output full doc)
                content = self._strip_document_preamble_from_section(content)
...
"""
    ...
    @staticmethod
    def _strip_document_preamble_from_section(content: str) -> str:
        """
        Strip document preamble and structure from section content.
        WriterAgent may incorrectly output full document structure (\\documentclass,
        \\usepackage, \\begin{document}, \\end{document}) in section body.
        """
        if not content or not content.strip():
            return content
        import re
        # Extract content inside \begin{document}...\end{document} if present
        doc_match = re.search(
        ...
        # Remove \documentclass and \usepackage lines
        lines = []
  ...
  year = {{{year}}},
    ...
    async def _compile_pdf(
        ...
        - **Args**:
            ...
            - `template_path` (Optional[str]): Path to .zip template file; None for plain (article/plain) compilation
        ...
        """
        # Treat invalid template_path as None -> fall back to plain (article/plain) compilation
        effective_template_path = template_path if (template_path and os.path.exists(template_path)) else None
        if template_path and not effective_template_path:
            print(f"[MetaDataAgent] template_path not found: {template_path}, falling back to plain")
        print(f"[MetaDataAgent] Phase 4: Compiling PDF with template: {effective_template_path or 'plain (no template)'}")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=600.0) as client:
                response = await client.post(
                    f"{EASYPAPER_BASE_URL}/agent/typesetter/compile",
                    json={
                        ...
                        "payload": {
                            ...
                            "template_config": {
                                "paper_title": paper_title,
                                **(
                                    {"document_class": "article", "bib_style": "plain", "citation_style": "cite"}
                                    if not effective_template_path
                                    else {}
                                ),
                            },
    ...
    async def _call_vlm_review(
        ...
        try:
            async with httpx.AsyncClient(timeout=300.0) as client:
                response = await client.post(
                    f"{EASYPAPER_BASE_URL}/agent/vlm_review/review",
                    json={
                        "pdf_path": pdf_path,
    ...
    async def _create_paper_plan(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=120.0) as client:
                response = await client.post(
                    f"{EASYPAPER_BASE_URL}/agent/planner/plan",
                    json={
                        "title": metadata.title,
                        "idea_hypothesis": metadata.idea_hypothesis,
    ...
    async def _call_reviewer(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
                    f"{EASYPAPER_BASE_URL}/agent/reviewer/review",
                    json=payload,
                )

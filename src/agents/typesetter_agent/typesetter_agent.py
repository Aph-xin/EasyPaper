...
class TypesetterAgent(BaseAgent):
    ...
    async def setup_workspace(self, state: TypesetterAgentState) -> Dict[str, Any]:
        ...
        if template_path and os.path.exists(template_path):
            if template_path.endswith('.zip'):
                ...
                # Patch templates that require inconsolata (e.g. COLM) to make it optional
                self._patch_optional_inconsolata(work_dir)
    ...
    def _patch_optional_inconsolata(self, work_dir: str) -> None:
        """
        Patch .sty files to make inconsolata optional (fallback to courier).
        COLM and similar templates require inconsolata which may not be installed.
        """
            ...
            for name in files:
                ...
                try:
                    with open(path, 'r', encoding='utf-8', errors='replace') as f:
                        content = f.read()
                    if 'inconsolata' not in content.lower():
                        continue
                    # Replace RequirePackage{inconsolata} and usepackage{inconsolata} with optional fallback
                    new_content = re.sub(
                        r'\\RequirePackage(\[[^\]]*\])?\{inconsolata\}',
                        r'\\IfFileExists{inconsolata.sty}{\\RequirePackage{inconsolata}}{\\RequirePackage{courier}}',
                        content,
                        flags=re.IGNORECASE,
                    )
                    new_content = re.sub(
                        r'\\usepackage(\[[^\]]*\])?\{inconsolata\}',
                        r'\\IfFileExists{inconsolata.sty}{\\usepackage{inconsolata}}{\\usepackage{courier}}',
                        new_content,
                        flags=re.IGNORECASE,
                    )
                    if new_content != content:
                        with open(path, 'w', encoding='utf-8') as f:
                            f.write(new_content)
                        logger.info("typesetter.patched_inconsolata file=%s", name)
                except Exception as e:
                    logger.warning("typesetter.patch_inconsolata_failed file=%s error=%s", name, e)
    ...
    def _strip_document_preamble_from_section(content: str) -> str:
        ...
        \\usepackage, \\begin{document}, \\end{document}) in section body. These must be
        removed to avoid nested documents and missing-package errors (e.g. inconsolata).
        """
        ...
        # Remove \documentclass and \usepackage lines (preamble in section body)
        lines = []
    ...
    def _write_section_files(
        ...
        if "abstract" in sections and sections["abstract"].strip():
            ...
            abstract_content = self._strip_document_preamble_from_section(abstract_content)
